// ═══════════════════════════════════════════════════════════════════════
// POTISSE — members-view.js  Push 9  (Tabbed Member View)
// 12 pestanas | Lazy load Orders/NFC/Post | Sin scroll infinito
// Generado: 2026-08-06 16:15
// ═══════════════════════════════════════════════════════════════════════
...
export async function renderMemberDetail(root, customerId, initialTab = 'overview') {
  ...
  try {
    const [profileData, alertsData, syncStatusData] = await Promise.all([
      ...
      <div class="overview-grid">
        ...
        <div class="overview-main">
          ...
          <div class="field-grid" style="max-width: 100%;">
            ...
            <div class="field-value-readonly">${fieldDisplay(id.email)}</div>
            <div class="field-label">Phone</div>
            <div class="field-value-readonly">${fieldDisplay(id.phone)}</div>
            <div class="field-label">Address</div>
            <div class="field-value-readonly">${fieldDisplay(id.address_line1 || id.address)}</div>
            <div class="field-label">City</div>
            <div class="field-value-readonly">${fieldDisplay(id.city)}</div>
    ...
    <div class="tab-panel-inner">
      <section class="member-block" style="padding-top:0; border-bottom:none;">
        ...
        <div class="field-grid">
          ...
          <div class="field-value readonly-field">${fieldDisplay(id.first_name)}</div>
          <div class="field-label">Last name</div>
          <div class="field-value readonly-field">${fieldDisplay(id.last_name)}</div>
          <div class="field-label">Email</div>
          <div class="field-value readonly-field">${fieldDisplay(id.email)}</div>
          <div class="field-label">Phone</div>
          <div class="field-value readonly-field">${fieldDisplay(id.phone)}</div>
          <div class="field-label">Address</div>
          <div class="field-value readonly-field">${fieldDisplay(id.address_line1 || id.address)}</div>
          <div class="field-label">Apartment / Floor</div>
          <div class="field-value readonly-field">${fieldDisplay(id.address_line2)}</div>
          <div class="field-label">City</div>
          <div class="field-value readonly-field">${fieldDisplay(id.city)}</div>
          <div class="field-label">Province</div>
          <div class="field-value readonly-field">${fieldDisplay(id.province)}</div>
          <div class="field-label">Postal Code</div>
          <div class="field-value readonly-field">${fieldDisplay(id.postal_code)}</div>
          <div class="field-label">Country</div>
...
function handleFieldClick(el) {
  ...
  const FIELD_CONFIG = {
    first_name: { type: 'text', value: currentProfile.identity.first_name, label: 'first name', backendField: 'first_name' },
    last_name: { type: 'text', value: currentProfile.identity.last_name, label: 'last name', backendField: 'last_name' },
    phone: { type: 'text', value: currentProfile.identity.phone, label: 'phone', backendField: 'phone' },
    address: { type: 'address', value: currentProfile.identity.address_line1 || currentProfile.identity.address, label: 'address', backendField: 'address_line1' },
    address_line2: { type: 'text', value: currentProfile.identity.address_line2, label: 'apartment / floor', backendField: 'address_line2' },
    city: { type: 'text', value: currentProfile.identity.city, label: 'city', backendField: 'city' },
    province: { type: 'text', value: currentProfile.identity.province, label: 'province', backendField: 'province' },
    postal_code: { type: 'text', value: currentProfile.identity.postal_code, label: 'postal code', backendField: 'postal_code' },
    ...
    email: { type: 'text', value: currentProfile.identity.email, label: 'email', backendField: 'email', sensitive: true }
  };
  ...
  startFieldEdit(el, {
    ...
    onFinish: (newValue) => {
      ...
      if (config.sensitive) {
        handleEmailEdit(trimmed);
      } else if (field === 'address' && extractedComponents) {
        const fieldsToSave = {
          [config.backendField]: trimmed || null,
          city: extractedComponents.city || currentProfile.identity.city || null,
          province: extractedComponents.province || currentProfile.identity.province || null,
          postal_code: extractedComponents.postal_code || currentProfile.identity.postal_code || null,
          country: extractedComponents.country || currentProfile.identity.country || null
        };
        handleIdentitySaveMulti(fieldsToSave, 'address, city, province, postal code, country');
      } else {
...
function isAddressSyncError(err) {
  return err?.status === 502 ||
    (err?.data?.error && String(err.data.error).startsWith('shopify_address_'));
}
...
async function handleIdentitySave(backendField, value, label) {
  ...
  try {
    ...
    if (isAddressSyncError(err)) {
      toast('Address could not be saved to Shopify. Please retry.', 'error');
    } else {
...
async function handleIdentitySaveMulti(fields, label) {
  ...
  if (!ok) { render(); return; }
  try {
    await api.post(`admin/members/${currentCustomerId}/edit`, fields);
    toast('Saved.', 'success');
    await refresh();
  } catch (err) {
    if (isAddressSyncError(err)) {
...
async function handleEmailEdit(newEmail) {
  if (!isValidEmail(newEmail)) {
    const emailField = currentRoot.querySelector('[data-field="email"]');
    if (emailField) {
      emailField.innerHTML = `<input type="text" value="${escapeHtml(newEmail)}"><div class="field-error" style="color: var(--danger); font-size: var(--size-xs); margin-top: 4px;">Enter a valid email address</div>`;
      const input = emailField.querySelector('input');
      input.focus();
      input.addEventListener('keydown', (e) => {
        const isEnter = e.key === 'Enter' || e.keyCode === 13 || e.which === 13;
        const isEscape = e.key === 'Escape' || e.keyCode === 27 || e.which === 27;
        if (isEnter) { e.preventDefault(); handleEmailEdit(input.value); }
        else if (isEscape) { render(); }
      });
      input.addEventListener('blur', () => {
        if (isValidEmail(input.value)) handleEmailEdit(input.value);
        else render();
      });
    }
    return;
  ...
  const ok = await confirmModal(
    `Change email of ${escapeHtml(name)}`,
    `New email: ${escapeHtml(newEmail)}. This change will also propagate to Shopify. All future communications will go to the new email.`,
    'Confirm', false
  );
  if (!ok) { render(); return; }
  try {
    await api.post(`admin/members/${currentCustomerId}/edit`, { email: newEmail });
    toast('Saved.', 'success');
    await refresh();
  } catch (err) {
    toast(err.message || 'Save failed.', 'error');
...
function actionButtonsMarkup() {
  const hasAlert = !!activeAlertEntry;
  return `
    <button type="button" class="btn btn-sm btn-secondary" id="act-send-email">Send email</button>
    <button type="button" class="btn btn-sm btn-secondary" id="act-magic-link">Send magic link</button>
    <button type="button" class="btn btn-sm btn-secondary${hasAlert ? '' : ' btn-disabled'}" id="act-mark-alert" ${hasAlert ? '' : 'disabled title="No active alert"'}>Mark alert resolved</button>
    <button type="button" class="btn btn-sm btn-secondary" id="act-silencio-1">Send Silencio 1 manual</button>
    <button type="button" class="btn btn-sm btn-secondary" id="act-register-incidence">Register incidence</button>
    <button type="button" class="btn btn-sm btn-danger" id="act-emergency-session">Emergency session</button>
  `;
}



  // v6.9.2: soporta tanto boton Retry sync como boton Sync now del banner

  const originalText = btn ? btn.textContent : '';
  if (btn) { btn.textContent = 'Retrying...'; btn.disabled = true; }
  try {

    // v6.9.2: validar response.ok antes de toast de exito
    if (!result || !result.ok) {
      throw new Error(result?.error || 'Sync failed: server returned error');
    }
        toast('Sync successful. Shopify is now up to date.', 'success');
    await refresh();
  } catch (err) {
    toast(err.message || 'Sync retry failed.', 'error');
    if (btn) { btn.textContent = originalText; btn.disabled = false; }
  }
}

// ── Auto-sync + Undo system ──────────────────────────────────────────
const undoSnapshots = new Map(); // customerId -> { fields: {}, timeoutId, toastEl }
const recentEdits = new Map();   // customerId -> { field, timestamp }

function recordEdit(customerId, field) {
  recentEdits.set(customerId, { field, timestamp: Date.now() });
}

function hasRecentEditOnFields(customerId, divergedFields, windowMs = 5 * 60 * 1000) {
  const edit = recentEdits.get(customerId);
  if (!edit) return false;
  const elapsed = Date.now() - edit.timestamp;
  if (elapsed > windowMs) return false;
  return divergedFields.includes(edit.field);
}
...
function showUndoToast(customerId, fields) {
  ...
  undoSnapshots.set(customerId, { fields, timeoutId, toastEl });
