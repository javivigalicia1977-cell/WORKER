    ...
    <div class="tab-panel-inner">
      <section class="member-block" style="padding-top:0; border-bottom:none;">
        ...
        <div style="margin-bottom:12px;font-size:var(--size-xs);color:var(--muted);text-align:right;">
          <a href="https://admin.shopify.com/store/potisse-shop/customers/${customerId}" target="_blank" style="color:var(--primary);text-decoration:underline;">Edit in Shopify Admin →</a>
        </div>
...
async function handleEmailEdit(newEmail) {
  ...
  const ok = await confirmModal(
    ...
    `New email: ${escapeHtml(newEmail)}. This change will also managed in Shopify. All future communications will go to the new email.`,
    'Confirm', false
  );
  if (!ok) { render(); return; }
  try {
