// stock.js — Shell tab Stock (v1.3 skeleton C.5-B.3+)
// Sub-tabs: overview (placeholder), suppliers (live), catalog (live), batches (placeholder)
...
async function loadSubTabContent(root) {
  ...
  if (currentSubTab === 'suppliers') {
    ...
    try {
      const mod = await import('/js/tabs/stock/catalog-view.js?v=1');
      container.innerHTML = mod.renderCatalogView();
