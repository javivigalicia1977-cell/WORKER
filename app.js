...
window.fetch = async function(url, options = {}) {
  if (typeof url === 'string' && url.includes('/api/proxy/admin/')) {
    ...
    const criticalPattern = /api\/proxy\/admin\/(nfc-card$|emergency-session|system\/domain-ssl|orders\/\d+\/fulfill|nfc-cards\/\w+\/(assign|unassign|mark-lost))/;
...
/* ── Sidebar badges honestos (C.4 fix intermedio) ─────────────────────── */
async function updateIncidenceBadge() {
  ...
  try {
    ...
    console.error('[IncidenceBadge] error:', e);
  }
}

async function updateAccessAlertBadge() {
...
function updateAllBadges() {
  ...
  updateAccessAlertBadge();
}

updateAllBadges();
setInterval(updateAllBadges, 30000);
window.addEventListener('hashchange', updateAllBadges);
window.updateIncidenceBadge = updateIncidenceBadge;
window.updateAccessAlertBadge = updateAccessAlertBadge;
